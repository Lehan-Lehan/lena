...
> LEHAN-MD MINI 🔥`;
    ...
    const captionText = `*👋LEHAN-MD MINI💗🍒*´
Download Menu 
* .song
* .fb
* .tiktok
...
case 'ownermenu': {
    ...
    const captionText = `*👋HY I m LEHAN-MD MINI 💗🍒*´
Download Menu 
* .song
* .fb
* .tiktok
...
> LEHAN-MD MINI💚👨‍🔧`;
    ...
    await socket.sendMessage(m.chat, {
        image: { url: "https://files.catbox.moe/nev2ek.jpg" },
        caption: captionText.trim(),
        footer: 'LEHAN-MD MINI🔥',
        buttons: templateButtons,
        headerType: 1
    }, { quoted: msg });
...
case 'ping': {
    ...
    const ownerName = 'LEHAN-MD ';
    const organization = '*LEHAN-MD MINI OWNER 👾*';
        ...
        if (!id) {
           ...
           if(!searchResults?.results?.length) return await socket.sendMessage(from, {
             ...
             text: '*📛 Please enter valid you tube song name or url.*'
                ...
                const caption = `*🎧 \`LEHAN-MD MINI BOT SONG DOWNLOADER\`*\n\n` +
		  `*┏━━━━━━━━━━━━━━━*\n` +
	      `*┃ 📌 \`тιтℓє:\` ${title || "No info"}*\n` +
...
case 'fb': {
  ...
  try {
    ...
    const url = args[0];
    const caption = `🎬💚 *LEHAN-MD MINI FB DOWNLOADER*
...
case 'tiktokdl': {
    try {
        ...
        const titleText = '*LEHAN-MD MINI TIKTOK DOWNLOADER*';
        const content = `┏━━━━━━━━━━━━━━━━\n` +
...
නම: LEHAN-MD MINI
හැදුවෙ: 𝙻𝙴𝙷𝙰𝙽 𝚖𝚍 𝚖𝚊 𝚝𝚑𝚊𝚖𝚊𝚒
තව කවෘ හරි මම 𝙻𝙴𝙷𝙰𝙽 𝚃𝙷𝙴𝙹𝙰𝙽𝙰කියලා කිව්විත් එයාට ගරු කරලා කතා කරන්න, තව සන්වර විදිහට හොදට හදලා ඔන බන්
...
case 'active': {
    ...
    let message = `*⚡LEHAN-MD MINI ACTIVE BOT LIST ⚡*\n`;
    message += `━━━━━━━━━━━━━━━\n`;
